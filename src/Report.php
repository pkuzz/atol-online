...
class Report implements ResponsePart {
    ...
    /**
     * @param array $array every item in array is optional except uuid and status -- they are mandatory. If they are absent -- method should (and will) fail.
     * @return Report
     */
    public static function fromArray(array $array): Report {
        $result = new Report();

        $dtTimestamp = DateTime::createFromFormat('d.m.Y H:i:s', $array['timestamp'] ?? 'now');

        $result->uuid = $array['uuid'];
        $result->status = $array['status'];

        $result->timestamp = $dtTimestamp === false ? new DateTime() : $dtTimestamp;
