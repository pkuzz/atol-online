...
class ReportPayload implements ResponsePart {
    ...
    public static function fromArray(array $array): ReportPayload {
        $dtReceipt = DateTime::createFromFormat('d.m.Y H:i:s', $array['receipt_datetime'] ?? 'now');

        $result = new ReportPayload();

        $result->fiscalReceiptNumber = $array['fiscal_receipt_number'] ?? 0;
        $result->shiftNumber = $array['shift_number'] ?? 0;
        $result->receiptDateTime = $dtReceipt === false ? new DateTime() : $dtReceipt;
        $result->total = $array['total'] ?? 0.0;
        $result->fnNumber = $array['fn_number'] ?? '';
        $result->ecrRegistrationNumber = $array['ecr_registration_number'] ?? '';
        $result->fiscalDocumentNumber = $array['fiscal_document_number'] ?? 0;
        $result->fiscalDocumentAttribute = $array['fiscal_document_attribute'] ?? 0;
